...
def get_partial_corr_scores(fluor):
    ...
    tF = thresh(dF)
    rF = regularize(tF)
    pca = PCA(whiten=True, n_components = int(fluor.shape[0]*0.8) ).fit(rF.T)
    #pca = PCA(whiten=True, n_components = 100).fit(rF.T)
...
#changed 1.2e6 to 1.2e4 for the small dataset
def pairwise_prep_tuple_partialcorr(fluor_, connect, pcorr_, num_images_target=1.2e4, represent=50, num_samples = 320):
    """
...
def valid_eval_tfomics_partialcorr(nnt, val_dat, pcorr, N=14, fragLen=330):
    ...
    startgap = np.ceil((val_dat.shape[1] - fragLen)/float(N)).astype('int')
