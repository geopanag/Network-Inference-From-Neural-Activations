...
higher_order_causation = 50 
def find_previous_impulse(impulse,causes):
    for c in causes:
        if((c>(impulse-higher_order_causation)).any()):
            return False
    return True
...
for f in range(1,7) :
    ...
    for n1 in range(0,len(spikes_idx)):
        ...
        for s in spikes:
            ...
            #--------------- Keep the exact preceding spikes
            possible_causes = map(lambda x: x[np.where(x>=(s-higher_order_causation))], possible_causes)
            ...
            #--------------- Subset to the possible cuases that are before spike s
            possible_causes_for_s_impulses = map(lambda x: x[np.where(x<s)], possible_causes)
            
            #dt[[find_previous_impulse(sn1,s) for sn1 in possible_causes]]=0
            impulses_times.append(np.append(possible_impulses,n1))
            #-------------- If the impulse are less then DT from the previous spike, do not assign them as impulses to s
    ...
    impulses_times.to_csv("../Data/small/impulse_times_"+str(f)+".csv",index=False)
    impulses_times_triangles.to_csv("../Data/small/impulse_times_triangles_"+str(f)+".csv",index=False)
    
    
    #---- Turn spike-impulse time to score
    impulses_times_scores = impulses_times.iloc[:,0:100].apply(lambda x:np.exp(1.0/(x))-1,axis=0)
    impulses_times_scores['label'] = impulses_times.iloc[:,100]
    impulses_times_scores[np.isinf(impulses_times_scores)] = 0
    ...
    #----- Store the versions with score as well
    impulses_times_scores = pd.DataFrame(np.asarray(impulses_times_scores))
    impulses_times_scores.to_csv("../Data/small/impulses_times_scores_"+str(f)+".csv",index=False)
    ...
    log.write("calculate impulse responses "+str(f)+" :" +str(t))
    log.write("\n")    
...
for f in range(1,7) :
    ...
    final = []
    impulses_times = pd.read_csv("../Data/small/impulse_times_"+str(f)+".csv")
    ...
    #------------ Examine  only for the first neuron
    ...
    final.to_csv("../Data/small/influence_times_"+str(f)+".csv",index=False)
    
    t = time.time() - start_time
    log.write("create the dataset scores "+str(f)+" :" +str(t))
    log.write("\n")    
...
for f in range(1,7) :
    ...
    test = pd.read_csv("../Data/small/influence_times"+c+"_"+str(f)+".csv")
    
    train = pd.DataFrame()
    for j in range(1,7):
        if(f!=j):
            train = train.append(pd.read_csv("../Data/small/influence_times"+c+"_"+str(j)+".csv"))
    ...
    #clf.score(test.as_matrix(test.columns[0:4]),test.iloc[:,4])
...
results.to_csv("../Data/results/influence_times_"+c+".csv",index=False)
    
t = time.time() - start_time
log.write("influence classification time with SVM" +c+" "+str(t))
log.write("\n")

  
with open('../Data/results/influence_network_validation.txt', 'w') as f:
        f.write("AUC: "+np.mean(aucs)+"\n")
        f.write("PRC: "+np.mean(prcs))
